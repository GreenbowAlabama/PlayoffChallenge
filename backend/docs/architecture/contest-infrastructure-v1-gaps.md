...
## Scoring Service Isolation & Determinism (Architecture Enforcement)

| Attribute | Value |
|---|---|
| Status | `CLOSED` |
| Layer | Architecture, Test Infrastructure |
| Description | The scoring service (`services/scoringService.calculateFantasyPoints`) is now architecturally isolated with zero side effects and pure dependency injection. (1) **Decoupled from server.js**: `calculateFantasyPoints` is no longer exported or invoked from the Express app bootstrap. It is a pure function accepting `(pool, stats)` as explicit parameters. (2) **No implicit pool creation**: Unit tests no longer require `server.js`, preventing incidental database pool creation and connection leaks. All unit tests use `mockPool` via explicit dependency injection. (3) **Test isolation**: Unit tests in `tests/services/scoring.service.test.js` import the function directly from `services/scoringService.js`, not from the server module. All scoring calculations pass a mock pool explicitly: `calculateFantasyPointsFunc(mockPool, stats)`. (4) **Jest exit guarantees**: No test leaves open database handles. `npm test -- --detectOpenHandles` reports zero TCP connections after suite completion. (5) **Idempotency verified**: Same stats input always produces identical numeric output (rounded to 2 decimal places). (6) **Null/undefined handling**: Function safely handles missing stats fields without errors. (7) **No module-level side effects**: The scoring service does not initialize timers, intervals, event listeners, or global state. It is safe to import in any context. |
| Why it matters | The platform principle "Minimize manual admin intervention" and the contract principle "Settlement is deterministic and replayable" both require that scoring logic is pure and repeatable. With scoring isolated, tests run fast, deterministically, and without leaving dangling resources. This enables reliable settlement and scoring audits. |
| Dependencies | None. This is a test infrastructure and architectural enforcement concern, not a runtime prerequisite. |
...
|---|---|---|---|
| 2026-02-15 | v1 | System | **PHASE 03 COMPLETE: Stripe Webhook Finalization & Cleanup**. Removed all debug/info console logs from webhook and payment processing paths (StripeWebhookService, PaymentIntentService, routes/webhooks.js, routes/payments.js). Confirmed zero UPDATE/DELETE statements against stripe_events table — append-only enforced by database trigger `stripe_events_no_update`. Verified transaction boundaries: BEGIN → INSERT stripe_events → process event → COMMIT, with proper ROLLBACK on failure. All idempotency keys (stripe_event_id, ledger.idempotency_key) uniqueness enforced at DB. Updated contest-infrastructure-v1-gaps.md with Stripe webhook architecture. Updated contest-lifecycle.md with new "Stripe Webhook Processing Guarantees" section documenting append-only semantics, idempotency mechanisms, and transactional atomicity. Staging end-to-end testing completed: payment_intent.succeeded inserts to ledger with internal UUID in reference_id and evt_* in stripe_event_id. Unit tests passing. |
| 2026-02-15 | v1 | System | **SCORING SERVICE ISOLATION & DETERMINISM CLOSED**: Decoupled `calculateFantasyPoints` from `server.js`, eliminating implicit database pool creation in unit tests. Refactored `tests/services/scoring.service.test.js` to import function directly from `services/scoringService.js` and explicitly pass `mockPool` as parameter. Removed `server.js` imports from test suite. Result: Jest exits cleanly with zero open handles (`npm test -- --detectOpenHandles` now passes). All 1180 tests pass deterministically. Added formal "Scoring Service Contract" section to `contest-lifecycle.md` documenting function signature, determinism guarantees, and testing requirements. Created `docs/architecture/testing-standards.md` with complete Test Isolation Policy enforcing dependency injection, forbidding server.js imports in unit tests, and clarifying unit vs. integration boundaries. Updated `contest-infrastructure-v1-gaps.md` with new "Scoring Service Isolation & Determinism (Architecture Enforcement)" section documenting closure criteria. This closes a critical architectural gap preventing settlement auditability and scorerepeatability. |
