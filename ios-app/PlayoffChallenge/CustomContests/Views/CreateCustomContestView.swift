...
struct CreateCustomContestView: View {
            ...
            if let structure = viewModel.selectedPayoutStructure {
                ForEach(Array(viewModel.payoutPreview.enumerated()), id: \.offset) { _, line in
                    HStack {
                        ...
                        Text(String(format: "$%.2f", line.amount ?? 0.0))
                            .monospacedDigit()
                    }
                }
            } else {
    ...
    private var statusSection: some View {
        ...
        if let result = viewModel.publishResult {
            Section("Share Link") {
                VStack(alignment: .leading, spacing: 8) {
                    Text(result.joinURL.absoluteString)
                        .font(.caption)
                        .foregroundStyle(.secondary)
                    Button {
                        UIPasteboard.general.string = result.joinURL.absoluteString
                    } label: {
...
private final class PreviewCustomContestCreator: CustomContestCreating {
    func loadTemplates() async throws -> [ContestTemplate] {
        return [
            ContestTemplate(
                ...
                description: "Golf challenge contest",
                defaultEntryFeeCents: 5000,
                allowedEntryFeeMinCents: 0,
                allowedEntryFeeMaxCents: 10000,
                defaultMaxEntries: 100,
                allowedPayoutStructures: [PayoutStructure.stub()]
            ),
            ContestTemplate(
                id: UUID().uuidString,
                name: "NFL Playoff Pool",
                description: "NFL playoff contest",
    ...
    func createDraft(templateId: UUID, name: String, settings: CustomContestSettings, payoutStructure: PayoutStructure, userId: UUID, lockTime: Date?) async throws -> Contest {
        try await Task.sleep(for: .seconds(1))
        return Contest.stub(contestName: name)
    }
}
